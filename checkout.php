...
if (isset($_SESSION['user_id'])) {
    $stmt = $pdo->prepare("SELECT email, phone, address FROM users WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    if (empty($errors)) {
        ...
        try {
            // Save order
            $stmt = $pdo->prepare("INSERT INTO orders (user_id, total, name, email, phone, address, payment_method) VALUES (?, ?, ?, ?, ?, ?, 'COD')");
            ...
            <div class="bg-white rounded-lg shadow-lg p-6">
                ...
                <form method="POST" class="mt-4">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" id="name" name="name" class="border rounded w-full px-4 py-2" value="<?php echo isset($user_info['name']) ? htmlspecialchars($user_info['name']) : ''; ?>" required>
                    </div>
                    <div class="mb-4">
                        <label for="email" class="block">Email</label>
                        <input type="email" id="email" name="email" class="border rounded w-full px-4 py-2" value="<?php echo isset($user_info['email']) ? htmlspecialchars($user_info['email']) : ''; ?>" required>
                    </div>
                    <div class="mb-4">
                        <label for="phone" class="block">Số Điện Thoại</label>
                        <input type="text" id="phone" name="phone" class="border rounded w-full px-4 py-2" value="<?php echo isset($user_info['phone']) ? htmlspecialchars($user_info['phone']) : ''; ?>" required>
                    </div>
                    <div class="mb-4">
                        <label for="address" class="block">Địa Chỉ</label>
                        <input type="text" id="address" name="address" class="border rounded w-full px-4 py-2" value="<?php echo isset($user_info['address']) ? htmlspecialchars($user_info['address']) : ''; ?>" required>
                    </div>
